...
public class Regex {
	...
	public static String normalizeLiteral(String str){
		...
		matcher = Pattern.compile(getLitStringRegex(str+"XXnormLit")).matcher(str);
		if(matcher.matches())
			return "\""+matcher.group("quote"+str+"XXnormLit")+"\"";
	...
	private static String getLitStringRegex(String str){
		return "(?<"+str+"XCV>"+QUOTE_MARK+")(?<quote"+str+">.+)\\k<"+str+"XCV>";
	}
	...
	public static String getOpRegex(String str){
		return "(?<"+str+">((?<Y"+str+">((increased)|(decreased)|(multiplied)|(divided)|(anded)|(ored)|(xored))) by )"+
					getValRegex(str+"XOp1")+")";
	}
	public static String getCompRegex(String str){
		return "(?<"+str+">was( not)?( ((less)|(greater)) than)?)";
	}
}